...
interface AdminViewProps {
  ...
  setPins: React.Dispatch<React.SetStateAction<{ [key in UserRole]?: string }>>;
  data: {
...
const AdminView: React.FC<AdminViewProps> = ({ waiter, t, lang, overrideRole, pins, setPins, data }) => {
  const effectiveRole = overrideRole || waiter.role;
  const isManager = effectiveRole === 'MANAGER';
  const isAdmin = effectiveRole === 'ADMIN';

  const managerTabs = ['dashboard', 'cashManagement', 'inventoryManagement', 'dailyReport'];
  const adminTabs = [
  ...
  const handleSettleCreditOrder = (order: Order) => {
    data.setOrders(prev => prev.map(o => o.id === order.id ? { ...o, status: 'paid' } : o));
  };
    ...
    <div className="h-full flex">
      ...
      <div className="flex-1 overflow-y-auto bg-gray-100">
        ...
        {activeTab === 'dailyReport' && (isAdmin || isManager) && (
         ...
         {activeTab === 'creditManagement' && isAdmin && (
          <CreditManagement
        ...
        {activeTab === 'restaurantSettings' && isAdmin && (
         ...
         {activeTab === 'tableSettings' && isAdmin && (
          <TableLayoutEditor
            tables={data.tables}
      ...
      <InventoryFormModal isOpen={inventoryModalOpen} onClose={() => setInventoryModalOpen(false)} onSave={handleSaveInventory} item={editingInventoryItem} t={t} currentUserRole={effectiveRole} />
      <ConfirmationModal 
